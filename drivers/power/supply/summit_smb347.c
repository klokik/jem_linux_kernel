...
{
	struct smb347_priv *priv = power_supply_get_drvdata(ps);
...
static const struct power_supply_desc smb347_ac_desc = {
	.name		= "smb347-ac",
	.type		= POWER_SUPPLY_TYPE_MAINS,
	.get_property	= smb347_get_ac_property,
	.properties	= smb347_charger_props,
	.num_properties	= ARRAY_SIZE(smb347_charger_props),
};

static const struct power_supply_desc smb347_usb_desc = {
	.name		= "smb347-usb",
	.type		= POWER_SUPPLY_TYPE_USB,
	.get_property	= smb347_get_usb_property,
	.properties	= smb347_usb_props,
	.num_properties	= ARRAY_SIZE(smb347_usb_props),
};
			...
			const struct i2c_device_id *id)
{
	...
	priv->ac = power_supply_register(&client->dev, &smb347_ac_desc,
					&ac_usb_cfg);
	if (IS_ERR(priv->ac)) {
		ret = PTR_ERR(priv->ac);
		dev_err(priv->dev,
	...
	priv->usb = power_supply_register(&client->dev, &smb347_usb_desc,
					&ac_usb_cfg);
	if (IS_ERR(priv->usb)) {
		ret = PTR_ERR(priv->usb);
		dev_err(priv->dev,
...
//	wake_lock_init(&priv->smb_wl, WAKE_LOCK_SUSPEND, "smb347_wakelock");
	...
	if (priv->i2c_client->irq != -1) {
		ret = request_threaded_irq(priv->i2c_client->irq,
			NULL, summit_smb347_irq, IRQF_TRIGGER_RISING | IRQF_ONESHOT,
			"smb347_irq", priv);
		if (ret) {
			dev_err(priv->dev, "Unable to set up threaded IRQ %d: %d\n",
				priv->i2c_client->irq, ret);
			goto err1;
		}
	}
...
err_out:
	...
	summit_smb347_remove(priv->i2c_client);
...
static int summit_smb347_resume(struct device *dev)
{
	struct smb347_priv *priv = dev_get_drvdata(dev);
...
MODULE_DEVICE_TABLE(of, summit_smb347_of_match);
...
static struct i2c_driver summit_smb347_i2c_driver = {
	.driver = {
			.name = "smb347",
			.of_match_table = summit_smb347_of_match,
			.pm = &summit_smb347_pm,
		},
